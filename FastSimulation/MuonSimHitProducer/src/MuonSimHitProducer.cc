...
MuonSimHitProducer::applyMaterialEffects(TrajectoryStateOnSurface& tsosWithdEdx,
 ...
 //Muon Brem in iron 
   ...
   theNewMomentum = theMuon.momentum() + energyLoss->deltaMom() + bremsstrahlung->deltaP_BremPhoton() + fac * deltaMom;
    
     }
    else{
//original contribution using only Energy losses
theNewMomentum = theMuon.momentum() + energyLoss->deltaMom() + fac * deltaMom;
